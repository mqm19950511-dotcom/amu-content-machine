...
- 📹 **视频/图文博主切换**：创作者表格按视频占比打标、可筛选（未深拉博主也有粗分类，来自搜索时的 vid/img 统计）
...
- 📋 **素材库**：收集任何好内容——手动粘贴导入，或用 `browser-extension/` 里的浏览器插件一键收录当前页面；未来灵感库会基于素材 + 创作者 + 你的资料生成新选题
- 🤖 **AI 内容流水线（DeepSeek）**：灵感库选题 →「用这个选题写草稿」→ 草稿页「AI 生成草稿」→「按意见修改」（支持一键按评审团意见改稿）→ 评审团页「开始评审」（6 评委并行独立打分）。**所有草稿按选题分别保存，下拉菜单切换**。提示词在 `prompts/*.md`，纯中文可手改；模板随仓库分发，`init.sh` 生成工作副本
